<div class="h-full flex flex-col">
    ...
    <div class="flex items-center gap-3 px-3 sm:px-6 py-3 sm:py-4 border-b border-neutral-800 shrink-0">
        <div class="flex gap-1 p-1 rounded-xl bg-neutral-900 border border-neutral-800 overflow-x-auto flex-1 min-w-0">
            ...
            <button wire:click="switchTab('points')"
                    class="flex items-center gap-2 px-4 py-2 rounded-lg text-sm font-medium transition-all duration-200 {{ $tab === 'points' ? 'bg-amber-500 text-neutral-950 shadow-lg shadow-amber-500/20' : 'text-neutral-400 hover:text-white hover:bg-neutral-800' }}">
                <svg class="w-4 h-4" fill="none" viewBox="0 0 24 24" stroke="currentColor">
            ...
            <a href="{{ route('client.support', ['slug' => $tenant->slug]) }}"
               class="flex items-center gap-2 px-4 py-2 rounded-lg text-sm font-medium transition-all duration-200 text-neutral-400 hover:text-white hover:bg-neutral-800">
                <svg class="w-4 h-4" fill="none" viewBox="0 0 24 24" stroke="currentColor">
    ...
    <div class="flex-1 overflow-y-auto">
        ...
        @if ($tab === 'points')
            <div class="p-4 sm:p-6">
                <div class="flex items-center gap-3 mb-4 sm:mb-6">
                    <h2 class="text-sm sm:text-lg font-bold">Meus Pontos</h2>
                </div>

                <div class="grid grid-cols-1 lg:grid-cols-2 gap-4 sm:gap-6">
                    {{-- Balance Card --}}
                    <div class="p-5 sm:p-6 rounded-2xl bg-emerald-500/5 border border-emerald-500/10">
                        ...
                        <div class="flex items-center justify-between p-5 rounded-xl bg-emerald-500/10 border border-emerald-500/20">
                            <div>
                                <p class="text-3xl sm:text-4xl font-black text-emerald-400">{{ number_format($myPointsBalance, 0, ',', '.') }}</p>
                                <p class="text-xs text-emerald-400/70 mt-1">pontos disponiveis</p>
                            </div>
                            <div class="text-right">
                                <p class="text-xs text-neutral-400">Ganhe {{ $pointsPercentageData }}% de volta</p>
                                <p class="text-[10px] text-neutral-500">em cada pedido</p>
                            </div>
                        </div>
                        @if ($myPointsBalance > 0)
                            <div class="mt-4">
                                <p class="text-xs text-neutral-400 mb-2">Valor em descontos:</p>
                                <p class="text-lg font-bold text-white">R$ {{ number_format($myPointsBalance / 100, 2, ',', '.') }}</p>
                            </div>
                            <a href="{{ route('menu.show', $tenant->slug) }}"
                               class="mt-4 inline-flex items-center gap-2 px-4 py-2.5 bg-amber-500 hover:bg-amber-400 text-neutral-950 font-semibold rounded-xl transition-all text-sm hover:scale-[1.02] active:scale-[0.98] w-full justify-center">
                    ...
                    {{-- How it Works --}}
                    <div class="p-5 sm:p-6 rounded-2xl bg-neutral-900/50 border border-neutral-800">
                        <h3 class="text-sm font-semibold text-neutral-300 mb-4 flex items-center gap-2">
                        ...
                        <ul class="space-y-3 text-sm text-neutral-400">
                            <li class="flex items-start gap-2">
                                <span class="w-5 h-5 rounded-full bg-emerald-500/10 text-emerald-400 flex items-center justify-center shrink-0 mt-0.5 text-xs font-bold">1</span>
                                <span>Faça pedidos e ganhe <strong class="text-emerald-400">{{ $pointsPercentageData }}%</strong> do valor de volta em pontos.</span>
                            </li>
                            <li class="flex items-start gap-2">
                                <span class="w-5 h-5 rounded-full bg-emerald-500/10 text-emerald-400 flex items-center justify-center shrink-0 mt-0.5 text-xs font-bold">2</span>
                                <span>Cada <strong class="text-emerald-400">100 pontos</strong> valem <strong class="text-amber-400">R$ 1,00</strong> de desconto.</span>
                            </li>
                            <li class="flex items-start gap-2">
                                <span class="w-5 h-5 rounded-full bg-emerald-500/10 text-emerald-400 flex items-center justify-center shrink-0 mt-0.5 text-xs font-bold">3</span>
                                <span>Use seus pontos como desconto no carrinho ao fazer um novo pedido.</span>
                            </li>
                        </ul>
                    </div>
                </div>
                ...
                {{-- Transaction History --}}
                @if ($myOrders->where('user_id', Auth::id())->count() > 0)
                    <div class="mt-4 sm:mt-6 p-4 sm:p-5 rounded-2xl bg-neutral-900/50 border border-neutral-800">
                        <h3 class="text-sm font-semibold text-neutral-300 mb-4 flex items-center gap-2">
                        ...
                        <div class="space-y-2">
                            @foreach ($myOrders->take(10) as $order)
                                @php
                                    $ptsTransaction = $order->pointsTransactions()->where('type', 'earned')->first();
                                @endphp
                                @if ($ptsTransaction)
                                    <div class="flex items-center justify-between p-3 rounded-xl bg-neutral-950 border border-neutral-800">
                                        <div class="flex items-center gap-3">
                                            ...
                                            <div>
                                                <p class="text-xs font-medium text-neutral-200">Pedido #{{ $order->id }}</p>
                                                <p class="text-[10px] text-neutral-500">{{ $order->created_at->format('d/m/Y') }}</p>
                                            </div>
                                        </div>
                                        <span class="text-sm font-bold text-emerald-400">+{{ $ptsTransaction->points }}</span>
                                    </div>
                                @endif
                            @endforeach
                            @if ($myPointsBalance === 0 && $myOrders->count() > 0)
                                <p class="text-center text-sm text-neutral-500 py-4">Nenhuma transacao de pontos ainda.</p>
                            @endif
                        </div>
                    </div>
                @endif
