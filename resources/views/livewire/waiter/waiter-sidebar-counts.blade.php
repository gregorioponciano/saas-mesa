<div wire:poll.3s class="flex flex-col h-full">
    ...
    <nav class="flex-1 overflow-y-auto px-3 py-4 space-y-1">
        @php
            $baseUrl = route('waiter.panel', ['slug' => $tenant->slug]);
            $currentTab = request()->query('tab', 'overview');
        @endphp

        <a href="{{ $baseUrl }}?tab=overview"
        ...
        <a href="{{ $baseUrl }}?tab=tables"
           class="flex items-center gap-3 px-4 py-2.5 rounded-xl text-sm font-medium transition-all duration-200 {{ $currentTab === 'tables' ? 'bg-amber-500/10 text-amber-400' : 'text-neutral-400 hover:text-white hover:bg-neutral-800/50' }}">
            <svg class="w-5 h-5 shrink-0" fill="none" viewBox="0 0 24 24" stroke="currentColor">
            ...
            @if ($occupiedTablesCount > 0)
                <span class="px-2 py-0.5 text-[10px] font-bold rounded-full bg-red-500/20 text-red-400 border border-red-500/30">{{ $occupiedTablesCount }}</span>
            @endif
        </a>

        <a href="{{ $baseUrl }}?tab=grid"
        ...
        <a href="{{ route('waiter.support', Auth::user()?->tenant?->slug) }}"
           class="flex items-center gap-3 px-4 py-2.5 rounded-xl text-sm font-medium transition-all duration-200 {{ request()->routeIs('waiter.support') ? 'bg-amber-500/10 text-amber-400' : 'text-neutral-400 hover:text-white hover:bg-neutral-800/50' }}">
            <svg class="w-5 h-5 shrink-0" fill="none" viewBox="0 0 24 24" stroke="currentColor">
