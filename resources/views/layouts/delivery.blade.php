...
<head>
    ...
    <title>{{ config('app.name') }} - {{ Auth::guard('delivery-web')->user()?->tenant?->name }}</title>
...
<body class="bg-neutral-950 text-white font-['Inter'] antialiased">
    <div x-data="{ sidebarOpen: false }" class="min-h-screen flex">
        ...
        <aside x-cloak
            ...
            @livewire('delivery.delivery-sidebar-counts', key('delivery-sidebar-counts'))
        </aside>
                ...
                <div class="flex items-center justify-between h-16 px-4 lg:px-8">
                    ...
                    <div class="flex items-center gap-3 ml-auto">
                        ...
                        <form method="POST" action="{{ route('delivery.toggle.availability') }}">
                            @csrf
                            <button type="submit"
                                    class="relative flex items-center gap-2 px-3 py-1.5 rounded-full text-xs font-semibold transition-all duration-300
                                    {{ $deliveryUser->status === 'active' ? 'bg-emerald-500/15 text-emerald-400 border border-emerald-500/30' : 'bg-neutral-800 text-neutral-400 border border-neutral-700' }}">
                                <span class="w-2 h-2 rounded-full {{ $deliveryUser->status === 'active' ? 'bg-emerald-400 shadow-[0_0_6px_rgba(52,211,153,0.6)]' : 'bg-neutral-500' }}"></span>
                                <span>{{ $deliveryUser->status === 'active' ? 'Online' : 'Offline' }}</span>
                            </button>
                        </form>
                        <form method="POST" action="{{ route('delivery.logout') }}">
                            @csrf
                            <button type="submit" class="p-2 rounded-xl hover:bg-neutral-800 transition-colors" title="Sair">
