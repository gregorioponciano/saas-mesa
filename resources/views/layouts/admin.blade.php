...
<head>
    ...
    <title>@yield('title', config('app.name') . ' - ' . (Auth::user()?->tenant?->name))</title>
...
<body class="bg-neutral-950 text-white font-['Inter'] antialiased">
    <div x-data="{ sidebarOpen: false }" class="min-h-screen flex">
        ...
        <aside x-cloak
            ...
            @livewire('admin.sidebar-counts', key('sidebar-counts'))
        </aside>
                    ...
                    <div class="flex items-center gap-3 ml-auto">
                        ...
                        @if (Auth::user()?->tenant?->isFree())
                            <a href="{{ route('subscription.checkout') }}"
                               class="px-4 py-1.5 text-xs font-semibold rounded-full bg-amber-500 hover:bg-amber-400 text-neutral-950 transition-all duration-200 hover:scale-105">
                                Fazer Upgrade
                            </a>
                        @endif
                    </div>
